...
export class SearchApplicantComponent {
  ...
  async fetch_proof_type() {
    ...
    if (this.proof_types == null) {
      ...
      await this.service
        .postservice('api/v1/get_doc_types_by_category', param)
        .toPromise()
        .then((data: any) => {
  ...
  async fetch_profiles() {
    ...
    await this.service
      .getServiceWithJsonParam('api/v0/get_profiles', param)
      .toPromise()
      .then((data) => {
        console.log(data);
        this.profiles = data;
  ...
  async fetch_districts() {
    ...
    if (this.districts == null) {
      ...
      await this.service
        .getServiceWithJsonParam('api/v0/get_districts', param, tag)
        .toPromise()
        .then((data: any) => {
          this.districts = data.sort(
            (a: any, b: any) => a.int_district_id - b.int_district_id
          );
          this.districts = this.service.save_to_cache(
            this.districts,
            'districts'
          );
        });
  ...
  filter_district_wise() {
    const selected_district = document.getElementById(
      'district'
    ) as HTMLInputElement;
    if (selected_district.value) {
      ...
      this.profiles = this.profiles.filter(
        (item: any) => item.dist_id == selected_district.value
      );
